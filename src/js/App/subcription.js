...
function save(){
    if($('form').valid()){
        ...
        gf.createSubscription(populate(),redirect);
    }
    return false;
}
...
function populate(){
    ccinfo = {};
    ccinfo.planID = config[campId].billing.planID;
    ccinfo.trialPackageID = config[campId].billing.trialPackageID;
    ccinfo.chargeForTrial = config[campId].billing.chargeForTrial;
    ccinfo.campaign_id = config[campId].billing.campaign_id;
